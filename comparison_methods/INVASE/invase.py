...
class Invase():
  ...
  def actor_loss(self, y_true, y_pred):
    ...
    actor_term = tf.reduce_sum(actor_out * K.log(y_pred + 1e-8) + (1 - actor_out) * K.log(1 - y_pred + 1e-8), axis=1)
    sparcity_term = tf.reduce_mean(y_pred, axis = 1)
  ...
  def build_actor(self):
    ...
    actor_model.add(Dense(self.actor_h_dim, activation=self.activation, kernel_regularizer=regularizers.l2(1e-3),
                          input_dim=self.dim))
    for _ in range(self.n_layer - 2):
      actor_model.add(Dense(self.actor_h_dim, activation=self.activation, kernel_regularizer=regularizers.l2(1e-3)))
    actor_model.add(Dense(self.dim, activation='sigmoid', kernel_regularizer=regularizers.l2(1e-3)))
  ...
  def build_critic(self):
    ...
    critic_model.add(Dense(self.critic_h_dim, activation=self.activation, kernel_regularizer=regularizers.l2(1e-3),
                           input_dim=self.dim))
  ...
  def build_baseline(self):
    ...
    baseline_model.add(Dense(self.critic_h_dim, activation=self.activation, kernel_regularizer=regularizers.l2(1e-3),
                             input_dim=self.dim))
    baseline_model.add(BatchNormalization())
    for _ in range(self.n_layer - 2):
      baseline_model.add(Dense(self.critic_h_dim, activation=self.activation, kernel_regularizer=regularizers.l2(1e-3)))
      baseline_model.add(BatchNormalization())
    baseline_model.add(Dense(self.label_dim, activation='softmax', kernel_regularizer=regularizers.l2(1e-3)))
  ...
  def train(self, x_train, y_train):
    ...
    for iter_idx in range(self.iteration):
      ...
      if self.model_type == 'invase':
        ...
        y_batch_final = np.concatenate((selection, np.asarray(critic_out), y_batch), axis=1)
