...
def VBEstep_private(sigma, X, y, exp_nat_params):
    # private version
    ...
    sensitivity = 1/np.float(N)
    std = sensitivity*sigma
    noise = np.random.normal(0,std,exp_suff_stats1.shape)
    ...
    std = (sensitivity/2)*sigma
    nse_mat = np.random.normal(0, std, exp_suff_stats2.shape)
