...
#######################
    ...
    if args.load_dataset and os.path.isfile(f"'../data/synthetic/{args.dataset}/dataset_{args.dataset}.npy"):
        ...
        elif args.dataset == "orange_skin_mean5":
        ...
        elif args.dataset == "nonlinear_additive":
    ...
    if "syn" in args.dataset:
        ...
        datatypes_tr, datatypes_tst, datatypes_tst_num_relevantfeatures = None, None, None

    if args.dataset=="adult_short" or args.dataset=="credit" or args.dataset=="intrusion":
        Xtst=X_test
        X=X_train
        ytst=y_test
        y = y_train

    return X, Xtst, y, ytst, x_tot, y_tot, datatypes_tr, datatypes_tst, datatypes_tst_num_relevantfeatures


def get_net(x_tot, args):
    if args.dataset=="intrusion":
        output_num = 4
    else:
        output_num = 2
...
def train(args, model, optimizer, criterion, X, Xtst, y, ytst):
    num_repeat = 1
        ...
        if 1:
            if args.testtype == "global":  # global test
                ...
                with open(global_json_path) as f:
                    data = json.load(f)
                try:
                    rank_str = data[args.dataset][met]
                except KeyError:
                ...
                if args.dataset == "adult_short" and met != 1 and met != 4:
                    dataset = "adult"
